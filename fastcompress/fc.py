...
This application works with reasonably recent versions of both Python 2 and
Python 3. It uses the six library to enable interoperability between the two.

Through the command line, simply:

 $ fc --decompress file.fc
 Decompressing......... Done
 See output: file
...
def s_decompress(x):
    ...
    def matrix_fix(lst):
        ...
        soln = numpy.round(numpy.dot(lst, numpy.linalg.inv(mkey))).tolist()
        print(soln)
        if soln[0][1] == 0:
            print(soln[0][0])
